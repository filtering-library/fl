...
public:
    ...
    -> decltype(f(VariateA(), VariateB()))
    {
        ...
        auto X_a = PointSet<VariateA, Size>(dim_a, point_count);
    ...
    void integrate_moments(
            ...
            typename SecondMomentOf<decltype(f(Variate()))>::Type& cov
        ) const
    {
        auto Z = PointSet<decltype(f(Variate())), number_of_points<Variate>()>();
     ...
     * \param [out] cov
     *              Integrated second centered moment \f$\mathbb{Cov}[f(x,y)]\f$.
     */
    template <
        typename Integrand,
        typename VarA,
